...
import { UserRecord } from './shared/user.model';
import { UserService } from './services/user.service';
import { UserCreateComponent } from './user-create/user-create.component';
import { UserUpdateComponent } from './user-update/user-update.component';
import { UserRoleAsignComponent } from './user-role-asign/user-role-asign.component';
...
export class UsersComponent {
  displayedColumns: string[] = ['id', 'username', 'name', 'email', 'phoneNumber', 'roles', 'actions'];
  table: TableData<UserRecord>;
  ...
  constructor(private service: UserService,
    ...
    this.table = new TableData<UserRecord>([
      {id: 'id', name: 'ID'},
      {id: 'name', name: 'Name'},
    ...
    this.table.dataSource = new MatMultiSortTableDataSource<UserRecord>(new MatMultiSort(), false);
  ...
  add(){
    ...
    instance.created.subscribe((id: string) => {
      this.getData();
      this.openSnackBar("User was created!");
      dialogRef.close();
    });
    instance.cancelled.subscribe(() => {
      this.openSnackBar("User creation was cancelled!");
      dialogRef.close();
    });
    instance.error.subscribe((error: any) => {
      ...
      if(error.status == 400){
        ...
        var messages = Object.keys(errors).map((key) => errors[key]);
        this.openSnackBar(messages.join(". "));
        }else{
  ...
  edit(id: string){
    ...
    instance.updated.subscribe((id: string) => {
      this.getData();
      this.openSnackBar("User was updated!");
      dialogRef.close();
    });
    instance.cancelled.subscribe(() => {
      this.openSnackBar("User update was cancelled!");
      dialogRef.close();
    });
    instance.error.subscribe((error: any) => {
  ...
  delete(id: string){
    this.confirm(this.doDelete.bind(this, id),
    {
      ...
      message: "Please confirm that you want to delete this user!",
  ...
  assignRoles(id: string){
    console.debug("assign roles", id);
    const dialogRef = this.dialog.open(UserRoleAsignComponent, {
      width: "600px",
      data: { id: id }
    });
    ...
    instance.updated.subscribe((id: string) => {
      this.getData();
      this.openSnackBar("User roles was updated!");
      dialogRef.close();
    });
    instance.cancelled.subscribe(() => {
      this.openSnackBar("User roles update was cancelled!");
      dialogRef.close();
    });
    instance.error.subscribe((error: any) => {
      this.openSnackBar("Error updating user roles!");
    });
  }
  //#endregion
}
