...
@Injectable({
  ...
  export class EventService {
    ...
    getEvents(filterText: string, status: string, sortColumns: string[] = [], sortDirections: string[]=[], 
        ...
        return this.http.post<TableQueryResult<TcpEvent>>(this.config.apiEndpoint + '/api/event/query', {
            query: filterText,
            page: page,
            pageSize: pageSize,
            status: status,
    ...
    processEvent(id: string) {
        return this.http.post<OperationResult>(this.config.apiEndpoint + `/api/event/${id}/process`, null);
    }

    abandonEvent(id: string) {
        return this.http.post<OperationResult>(this.config.apiEndpoint + `/api/event/${id}/abandon`, null);
    }
  }